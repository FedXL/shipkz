...
async def save_socket_user_mapping(socket_id, user_id):
    ...
    await redis_client.set(f'socket:{socket_id}', str(user_id))
    await redis_client.set(f'user:{user_id}', str(socket_id))
    await redis_client.close()


async def save_socket_to_redis_support(socket_id, user_id):
    redis_client = await connect_to_redis()
    await redis_client.set(f'socket_support:{socket_id}', str(user_id))
    await redis_client.set(f'user_support:{user_id}', str(socket_id))
    await redis_client.close()


async def get_user_id_by_socket_id_support(socket_id) -> int | None:
    my_logger.debug('start')
    redis_client = await connect_to_redis()
    user_id = await redis_client.get(f'socket_support:{socket_id}')
    await redis_client.close()
    try:
...
async def get_socket_id_by_user_id_support(user_id):
    redis_client = await connect_to_redis()
    socket_id = await redis_client.get(f'user_support:{user_id}')
    await redis_client.close()
    try:
...
async def delete_socket_from_redis_support(socket_id, user_id):
    """delete socket:user and user:socket from redis"""
    redis_client = await connect_to_redis()
    try:
        await redis_client.delete(f'socket_support:{socket_id}', user_id)
    except Exception as ER:
        my_logger.error(f"Cant to delete socket from redis {ER}")
    try:
        await redis_client.delete(f'user_support:{user_id}', socket_id)
    except Exception as ER:
