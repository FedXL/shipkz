...
from module_7.config.config_app import REDIS_URL, TEST_MODE_A
import uuid
...
async def connect_to_redis_pubsub() -> PubSub:
    rediss: Redis = await aioredis.from_url(REDIS_URL)
    pubsub: PubSub = rediss.pubsub()
    await pubsub.subscribe("news")
    return pubsub, rediss


async def connect_to_redis_pubsub_menu() -> PubSub:
    rediss: Redis = await aioredis.from_url(REDIS_URL)
    pubsub: PubSub = rediss.pubsub()
    await pubsub.subscribe("menu")
    return pubsub
